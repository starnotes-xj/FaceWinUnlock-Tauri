...
* **场景过滤**：读取注册表 `UNLOCK_SCENE`（默认 `"1,2,4"`）决定在哪些场景下激活，对不在列表中的场景返回 `E_NOTIMPL`。CredUI 场景保留 `consent.exe` 的 UAC 人脸提权；`credentialuibroker.exe` 使用 WebAuthn Ready/Active 守卫、owner 进程和明确标题语义分类，passkey/security key/PIN 设置直接跳过，Unknown Chrome/Edge 仅在监视器健康且 `CREDUI_BROWSER_PASSWORD_FILL=1` 时尝试人脸。
