...
- **`CSampleProvider.rs`** — `ICredentialProvider` impl; reads `UNLOCK_SCENE` registry key (default `"1,2,4"`) to filter which scenarios activate face unlock; starts `CPipeListener` in `Advise()`; returns `E_NOTIMPL` for unlisted scenarios (fixes browser PIN freeze, #118). `SetUsageScenario` 中通过 `dwflags & CREDUIWIN_GENERIC` 检测应用凭据弹窗（如 RDP），配合 `CREDUI_ALLOW_GENERIC` 注册表键进行过滤 (#114). `credentialuibroker.exe` 同时托管浏览器查看密码和 Google/passkey(WebAuthn) 验证，实测二者的 `CPUS_CREDUI`、`dwflags`、auth package、CLSID、`rgbSerialization` 完全一致，无法精准区分；当前默认先尝试面容识别，若 broker 场景超时或 `ReportResult` 拒绝凭据，则隐藏本 Provider 并交还 Windows PIN。`GetCredentialCount` 始终初始化所有输出指针防止 UB；autologon 由 `SharedCredentials.is_unlocked` 控制（脉冲信号，由 GetSerialization 消费重置），解决 UAC 场景多次调用导致 autologon 丢失问题 (#112).
...
| `CREDUI_ALLOW_BROKER` | `"0"` / `"1"` | Default `"1"`; compatibility switch for broker-hosted CredUI. Current behavior allows `credentialuibroker.exe` to try face first, then falls back to Windows PIN on timeout or credential rejection |
...
**Scenario 4 (CredUI) notes:** Chrome/Edge password reveal and Google/passkey WebAuthn verification both load the DLL inside `credentialuibroker.exe`, and diagnostic logging showed no stable Credential Provider field that distinguishes them. The DLL therefore lets broker-hosted CredUI try face first so browser password reveal still works; if passkey/WebAuthn rejects the submitted credential or face credentials do not arrive before `CREDUI_BROKER_FALLBACK_TIMEOUT`, the provider hides itself and returns control to Windows PIN. UAC face elevation via `consent.exe` remains supported. The log file is opened with `FILE_SHARE_READ | FILE_SHARE_WRITE` (append mode) so multiple host processes can write to the same `facewinunlock.log`; each startup logs its PID for disambiguation.
...
| GPU(OpenCL/OpenCL FP16) 后端在部分设备识别极慢/匹配失败（[issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3)）| `UI/src/views/Options.vue` + `UI/src/views/Faces/Add.vue` | OpenCL FP16 等 GPU 后端**加载会"成功"但运行时可能极慢（单次解锁~90s）+ 匹配不上**（GPU kernel 编译 / FP16 精度，加载阶段无法预知），表现为锁屏转圈不解锁、一致性检查黑屏卡住、占用偏高；CPU 后端正常。方案：**UI 警告 + 引导改回 CPU**（切 GPU 后端弹实验性警告，一致性检查出错提示改回 CPU）。**绝不**默认/强制 GPU 后端；**绝不**因「load_opencv_model 成功」就认定后端可用（#125 的加载回退检测覆盖不到「加载成功但运行时坏」）；自动回退因无法判别「首帧编译慢 vs 一直慢」且可能误判正常 GPU 设备，暂不做 |
