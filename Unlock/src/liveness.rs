...
impl PassiveLiveness {
    pub fn load(resources: &Path, backend_id: i32, target_id: i32) -> Result<Self> {
        ...
        // The UI enrollment flow owns `face_liveness.onnx` (facenox 98.20).
        // Keep the login PAD's MiniFASNetV2 model under an explicit name so the
        // two model contracts cannot be swapped accidentally.
        let secondary_path = resources.join(format!("face_liveness_mini_fasnet_v2.{extension}"));
