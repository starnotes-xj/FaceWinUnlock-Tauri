...
{
    namespace {
        constexpr wchar_t c_faceAuthorizationPipe[] =
            LR"(\\.\pipe\FaceWinUnlockPasskeyFaceAuth)";

        HRESULT RequestFaceWinUnlockAuthorization()
        {
            if (!WaitNamedPipeW(c_faceAuthorizationPipe, 5'000))
            {
                return HRESULT_FROM_WIN32(GetLastError());
            }

            wil::unique_hfile pipe(CreateFileW(
                c_faceAuthorizationPipe,
            ...
            constexpr char request[] = "authorize";
            ...
            RETURN_IF_WIN32_BOOL_FALSE(WriteFile(
                ...
                &bytesWritten,
                nullptr));
            RETURN_HR_IF(E_FAIL, bytesWritten != sizeof(request) - 1);
            ...
            std::string_view result(response.data(), bytesRead);
        ...
        wil::shared_cotaskmem_string userName)
    {
        ...
        try
        {
            ...
            // Keep the vault logically locked after successful local face verification.
            // The locked state tells the platform that the plugin already performed its
            // own user verification and prevents a second Windows Hello prompt.
            if (PluginCredentialManager::getInstance().GetVaultLock())
            {
                RETURN_IF_FAILED(RequestFaceWinUnlockAuthorization());
            ...
            DWORD uvCount = 0;
            RETURN_IF_FAILED(WebAuthNPluginGetUserVerificationCount(facewinunlock_plugin_guid, &uvCount));
        ...
        /* [out] */ __RPC__out PWEBAUTHN_PLUGIN_OPERATION_RESPONSE response) noexcept
    {
        ...
        try
        {
            ...
            *response = {};
            ...
            std::wstring keyNameStr = facewinunlock_plugin_key_domain;
