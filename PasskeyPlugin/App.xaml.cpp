...
void App::InitializeAppWindTitleBar()
{
    ...
    appWind.Title(L"FaceWinUnlock Passkey");
    ...
    std::wstring userName)
{
    ...
    credMgr.ReloadRegistryValues();
    {
        ...
        // 人脸门模式（vault 锁定）下，RequestFaceWinUnlockAuthorization 的人脸识别本身
        // 就是用户验证 + 操作确认，无需再弹"保存/登录"确认窗口让用户手点。
        // 因此 vault 锁定时一律静默自动 proceed（人脸失败仍会在 PerformUserVerification
        // 的 RETURN_IF_FAILED 处中止，安全性不受影响）。
        m_pluginOperationOptions.silentMode = credMgr.GetSilentOperation() || credMgr.GetVaultLock();
