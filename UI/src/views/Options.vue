<script setup lang="ts">
	...
	const uninstallDll = () => {
		ElMessageBox.confirm(
			'卸载 DLL、核心服务和 Passkey 插件包体，并还原主程序注册表。插件本地通行密钥会保留，重装后可继续使用；如需彻底删除，请在 Passkey 插件区选择彻底清除。程序将强制回到初始化页面。',
			'危险操作',
			{
				...
				<div v-if="activeTab === 'dll'" class="fade-in">
					...
					<div class="dll-settings">
						...
						<div class="setting-block">
							...
							<p class="sub muted">
								Passkey 插件依赖主程序的人脸识别服务（Unlock.exe）完成用户验证。卸载主程序或核心组件会保留插件本地通行密钥，重装后可继续使用；普通更新只更新 MSIX 包体。只有选择“彻底清除”才会删除本地通行密钥。
							</p>
						</div>
