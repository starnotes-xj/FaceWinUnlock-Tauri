...
- ✅ Google/passkey (WebAuthn) isolation: the Unlock service monitors system WebAuthn activity, while the generic Credential Provider skips passkey, security-key, and PIN-setup flows; Chrome/Edge password reveal and guarded fill continue to use face recognition
...
| v0.5.8 | 2026-07-05 | Face-management speedup + UI-not-responding fix + real camera device names (#17) + passkey uninstall hardening | **Fixed the UI going "Not Responding" after entering the main window / unlocking**: the auto update-check connects to GitHub (slow/unreachable in some regions), ureq had no timeout so it blocked to the TCP default (~20s+), and Tauri sync commands run on the main thread → the window froze. Added 8s timeouts to all update-check requests and made check_update / fetch_update_diff / apply_update async + spawn_blocking so blocking network never freezes the UI.<br />**Faster capture / consistency verification in face management**: the 3 ONNX models load in parallel (detector first to trigger DNN global init, then recognizer/liveness concurrently), and model loading runs in parallel with camera opening — first capture ~7s → ~2-3s.<br />**Camera dropdown now shows real device names** ([#17](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/17)): the reconstruction previously showed placeholders "Camera N"; on machines with several virtual cameras (NVIDIA Broadcast / OBS / VTubeStudio, etc.) users could not tell them apart and often picked a virtual camera → black frames → could not enroll/unlock. Switched to Media Foundation friendly-name enumeration (same order as OpenCV, so indices match); picking the integrated camera restores unlock.<br />**Passkey reinstall-without-re-registration hardening**: the vault registry material needed to unwrap credential private keys (HMACSecretInput / EncryptedVaultData under `HKCU\Software\FaceWinUnlock\PasskeyManager`) is now included in the backup/restore — previously only credentials.dat metadata was backed up, so if the vault registry was lost the credentials would show but signing would fail; uninstall also keeps the plugin package installed rather than deleting local passkeys when the system lacks `-PreserveApplicationData`. |
| v0.5.7 | 2026-07-03 | Enrollment fix: close-up / large-face photo import missed detection | **Fixed face not detected when importing a close-up / large-face photo** ([#20](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/20)): @ljili1 reported that scaling the same photo, 607×341 is detected but 608×342 and above are not (v0.5.6's downscale-to-1600 was not enough). Root cause: YuNet is sensitive to the **absolute pixel size of the face** — a large / close-up face in a big image exceeds the detectable range and is missed. Switched to **multi-scale detection**: try longest-side targets 1024 / 768 / 576 / 448 / 320 from large to small, using the first scale that detects a face (largest detectable scale first, for better SFace feature quality). Applied to both import and save paths; the live camera preview keeps single-scale detection. |
| v0.5.6 | 2026-07-03 | Camera-open slowness fix (Win10 MSMF ~40s) + enrollment image import fixes | **Fixed Win10 camera opening ~40s after recognition is triggered, breaking instant unlock** ([issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3)): a user reported v0.5.5 on Win10 waits ~40s for the camera after Ctrl+L (`unlock.log` shows MSMF open taking 41011ms / 39756ms), and the pre-warm was stuck on the same 40s. Root cause: OpenCV's MSMF (Media Foundation) backend enables hardware frame transforms by default, which hangs during init on some Win10 machines. The unlock service and UI now set `OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS=0` at startup to disable that path and restore normal open speed, while **keeping the MSMF backend** (same feature space as enrollment, so no falling back to DShow and its "face detected but never matches" issue).<br />**Fixed local-image import issues** ([#20](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/20)): (1) large images (e.g. 4K) caused missed / very slow detection → downscale to <=1600 on the longest side before detection; (2) non-ASCII (Chinese) image paths failed → read bytes then decode (Unicode-safe); (3) the previous enrollment's image/form persisted when adding a new face (keep-alive cached page) → reset per current action on each entry. |
| v0.5.5 | 2026-07-02 | Unlock animation removed, antivirus self-healing, instant unlock, stability hardening | **Removed the Windows Hello-style unlock animation completely** ([issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3), [#14](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/14), [#15](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/15), [#16](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/16), [#17](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/17)): the DComp/D2D/D3D11/DWrite rendering pipeline, frame resources, and registry switches were deleted. The Credential Provider DLL no longer depends on graphics libraries, avoiding old Win10 LogonUI load failures, lock-screen flicker, and sleep/resume spinner stalls at the root. The lock screen keeps a lightweight text status prompt instead.<br />**Fixed a Win10 camera-open delay where recognition was triggered but the camera opened about 40 seconds later**: offline analysis of #3 logs showed this was not model loading or a worker crash; v0.5.4's CAP_ANY-first path can block for a long time on some Win10 cameras. The unlock service now uses the same order as enrollment (`MSMF → DShow → Any`) and logs per-backend open times, avoiding both the old DShow-first feature mismatch and the CAP_ANY long block.<br />**Fixed Passkey Manager remaining after app uninstall** ([#17](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/17)): elevated uninstall now backs up each user's `credentials.dat`, then removes the MSIX package with `Remove-AppxPackage -AllUsers`, so the desktop user's Passkey Manager app is removed even when NSIS runs under an administrator context. KSP private keys are still kept, and metadata can be restored from backup after reinstall.<br />**Added runtime self-healing for antivirus false positives**: the installer compresses `opencv_world4120.dll` and `FaceWinUnlock-Server.exe` into `resources/runtime-backup.zip`, then registers the `FaceWinUnlockHealer` scheduled task to restore missing files on boot / logon / every 15 minutes and restart the core service. This mitigates Huorong / Defender deleting unsigned runtime files and causing "opencv_world4120.dll not found" or a missing background service.<br />**The installer tries to add a Windows Defender exclusion for the install directory**; with third-party antivirus products such as Huorong, manually add the install directory to the trusted / allow list.<br />**Instant unlock: camera pre-warm on lock**: when the lock screen appears the camera is pre-opened in the background (MSMF open is a ~2-3s hardware floor), so when the user moves the mouse to trigger recognition the camera is already ready and matching is near-instant, instead of waiting 2-3s to open it every time. It releases and turns off after 45s idle; if the pre-warm open fails (camera busy/absent) it stops retrying, and it yields when a shutdown/release is pending.<br />**Fixed auto-lock "enabled but no effect"**: the auto-lock toggle previously required clicking Save to persist; it now saves immediately on toggle, and skips the re-check while the screen is already locked (avoids camera contention with pre-warm and re-locking an already-locked screen).<br />**Enrollment consistency-verification fixes** ([#12](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/12)): fixed "model not loaded" after switching the inference backend (`ensureModelLoaded` now reloads idempotently); Back / Confirm changes stop verification and release the camera on leaving the page; re-capturing from the camera saves correctly.<br />**Fixed plugin showing "not installed" while installed**: a broken v-if chain in the Passkey plugin status caused the misreport; fixed.<br />**Passkey reinstall credential-recovery fallback**: querying plugin status now proactively backs up each user''s `credentials.dat`, so reinstalling after a third-party uninstaller (e.g. Geek) can restore metadata from backup, closing the v0.5.4 uninstall/reinstall key-loss gap.<br />**Update notification gains a "Go to download" button**: the new-version prompt is now clickable and opens the Release page directly. |
| v0.5.9 | 2026-07-14 | Resume, auto-lock, enrollment, Passkey, and broker fixes | **Fixed #26 LogonUI freezes after resume and delayed camera release after manual PIN unlock** by cancelling blocking synchronous I/O and moving release/CloseHandle work off the host UI thread. Camera pre-warm is now skipped when no enabled face records exist and remains blocked until the credential session changes after manual PIN release, preventing a stale client from reopening it.<br />**Fixed #27 auto-lock failures being logged as success** by verifying the locked desktop, retrying, and logging policy diagnostics with backoff on persistent failure.<br />**Improved black or slow enrollment camera startup** by loading models in the background on page entry, yielding the background pre-warm service through `ui_release`, and constructing models without holding the global camera-state lock. Physical cameras finish warm-up after three consecutive valid frames while virtual cameras retain a ten-frame fallback; MSMF hardware transforms remain disabled before first open with MSMF → DShow → Any fallback.<br />**Fixed stale Passkey registration and Geek-cleanup backup loss** by unregistering the authenticator and moving backups to `%ProgramData%\facewinunlock-tauri\PasskeyBackup`.<br />**Added UIA-free broker classification**: WebAuthn 1000-1008 activity is a hard veto; passkey, security-key, FIDO2, and PIN-setup flows skip the generic provider. Chrome/Edge password reveal and guarded fill use face recognition, while `CREDUI_BROWSER_PASSWORD_FILL=0` disables Unknown fallback. UIA and automatic PIN injection runtime code are removed. |
