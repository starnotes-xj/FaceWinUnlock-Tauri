...
| v0.5.9 | 2026-07-13 | Resume, auto-lock, enrollment, Passkey, and broker fixes | **Fixed #26 LogonUI freezes after resume and delayed camera release after manual PIN unlock** by cancelling blocking synchronous I/O and moving release/CloseHandle work off the host UI thread.<br />**Fixed #27 auto-lock failures being logged as success** by verifying the locked desktop, retrying, and logging policy diagnostics with backoff on persistent failure.<br />**Fixed black or slow enrollment camera startup** by asking the background pre-warm service to yield through `ui_release`, disabling MSMF hardware transforms before first open, and retaining MSMF → DShow → Any fallback.<br />**Fixed stale Passkey registration and Geek-cleanup backup loss** by unregistering the authenticator and moving backups to `%ProgramData%\facewinunlock-tauri\PasskeyBackup`.<br />**Added UIA-free broker classification**: WebAuthn 1000-1008 activity is a hard veto; passkey, security-key, FIDO2, and PIN-setup flows skip the generic provider. Chrome/Edge password reveal and guarded fill use face recognition, while `CREDUI_BROWSER_PASSWORD_FILL=0` disables Unknown fallback. UIA and automatic PIN injection runtime code are removed. |
