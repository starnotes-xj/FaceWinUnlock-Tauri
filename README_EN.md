...
| v0.5.3 | 2026-06-21 | Update-check false-positive fix + GPU backend experimental warning | **Fixed "already on the latest version but keeps prompting to update"**: the update check reads `CARGO_PKG_VERSION` (`UI/src-tauri/Cargo.toml`), but releases only bumped `tauri.conf.json` and missed Cargo.toml, so current_version lagged the release tag; bumped it and made CI sync **both** `tauri.conf.json` and `Cargo.toml` from the tag to prevent recurrence<br />**GPU (OpenCL / OpenCL FP16) inference backend experimental warning** ([issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3) follow-up): on some Windows 10 devices, selecting OpenCL FP16 makes face recognition extremely slow (~90s per unlock) and unable to match — the lock screen spins without unlocking, the consistency check freezes to a black screen, and resource usage is high — while CPU works fine; switching to a GPU backend in Preferences now clearly warns it is experimental and advises reverting to CPU on issues, with the same guidance shown when the consistency check errors<br />**OpenCL kernel tuning cache optimization**: the Unlock service and UI now set `OPENCV_OCL4DNN_CONFIG_PATH` to a persistent writable directory (ProgramData for the SYSTEM service, LOCALAPPDATA for the UI). Without it, OpenCV's ocl4dnn re-runs OpenCL kernel compilation + auto-tuning for conv layers on every `forward` (first inference on GPU/FP16 can take ~90s) and, since results aren't persisted, **repeats it on every unlock/recognition**; with it, only the first run tunes and subsequent runs load from cache instantly. Note: this only fixes the "slow" part; if a GPU's FP16 precision is insufficient and faces "can't match", reverting to CPU is still required<br />**Passkey uninstall now keeps credentials by default** ([issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3)): uninstall / full-update / reinstall no longer requires re-registration — the private key lives in the Microsoft Software KSP (key name `facewinunlock/<userId>`, per-user, never removed by MSIX uninstall), and credential metadata is now kept via `Remove-AppxPackage -PreserveApplicationData` plus an out-of-package backup/restore fallback; uninstall offers "keep credentials" / "purge" options, with core/NSIS uninstall defaulting to keep |
