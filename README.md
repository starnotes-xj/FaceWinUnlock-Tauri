...
- ✅ Google/passkey（WebAuthn）隔离：Unlock 订阅系统 WebAuthn 活动事件，通用 Credential Provider 在 passkey/security key/PIN 设置场景返回 `E_NOTIMPL`；Chrome/Edge 密码查看与受控填充继续保留人脸识别
...
| v0.5.8 | 2026-07-05 | 面容管理提速 + UI 未响应修复 + 摄像头真实设备名(#17) + passkey 卸载兜底 | **修复进主界面/解锁后 UI「未响应」**：进主界面自动查更新时国内直连 GitHub 慢/不通，而 ureq 无超时会卡到 TCP 默认超时(~20s+)，加上 Tauri 同步命令在主线程执行 → 窗口未响应。现给所有更新检查请求加 8s 超时，并把 check_update / fetch_update_diff / apply_update 改 async + spawn_blocking 移出主线程，连不上 GitHub 也不冻 UI。<br />**面容管理抓拍 / 一致性验证提速**：并行加载 3 个 ONNX 模型（detector 先触发 DNN 全局初始化，recognizer/liveness 再并行），并让「模型加载」与「摄像头打开」并行，首次抓拍约 7 秒 → ~2-3 秒。<br />**摄像头下拉显示真实设备名**（[#17](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/17)）：复原版此前只显示占位名「Camera N」，在装有多个虚拟摄像头（NVIDIA Broadcast / OBS / VTubeStudio 等）的机器上用户分不清、易选到虚拟摄像头 → 采集黑帧、无法录入/解锁。改用 Media Foundation 枚举真实设备名（与 OpenCV 同一枚举顺序，索引对应），选对集成摄像头即恢复。<br />**Passkey 卸载重装免重注册兜底加固**：把解开凭据私钥所需的「保险库注册表材料」（`HKCU\Software\FaceWinUnlock\PasskeyManager` 的 HMACSecretInput / EncryptedVaultData）纳入备份/恢复——此前只备份 credentials.dat 元数据，一旦保险库注册表丢失，重装后凭据能显示却签名失败；卸载还会在系统不支持 `-PreserveApplicationData` 时宁可保留插件包体也不删本地通行密钥。 |
| v0.5.7 | 2026-07-03 | 录入近景/大脸照片导入漏检修复 | **修复导入近景 / 大脸照片时检测不到人脸**（[#20](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/20)）：@ljili1 反馈同一照片缩放测试 607×341 能检到、608×342 起就检不到（v0.5.6 降采样到 1600 仍不够）。根因是 YuNet 对**人脸的绝对像素大小**敏感——近景大脸在大图上会超出可检测范围而漏检。改为**多尺度检测**：从大到小依次在 1024 / 768 / 576 / 448 / 320 最长边尺寸上尝试，取第一个检到人脸的尺度（优先最大可检尺度以保证 SFace 特征质量）。导入与保存路径均生效，摄像头实时预览保持单次检测不受影响。 |
| v0.5.6 | 2026-07-03 | 摄像头打开慢修复（Win10 MSMF 40 秒）+ 录入图片导入修复 | **修复 Win10 触发识别后摄像头约 40 秒才打开、秒解锁失效**（[issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3)）：有用户反馈 v0.5.5 在 Win10 上按 Ctrl+L 后要等约 40 秒摄像头才亮（`unlock.log` 实测 MSMF 打开耗时 41011ms / 39756ms），秒解锁的预热也被这 40 秒卡住。根因是 OpenCV 的 MSMF（Media Foundation）后端默认启用的**硬件帧变换（HW transforms）在部分 Win10 上初始化挂起**。解锁服务与 UI 均在启动时设置 `OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS=0` 关闭该路径，让 MSMF 打开恢复正常速度——同时**保留 MSMF 后端**（与录入端同一特征空间，不会退回 DShow 造成"检测到脸但匹配不上"）。<br />**修复导入本地图片相关问题**（[#20](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/20)）：① 4K 等大图导致人脸漏检 / 极慢（用户所说"像素问题"）→ 加载后按最长边降采样到 ≤1600 再检测；② 图片路径不支持中文 → 改为按字节读取后解码（Unicode 路径安全）；③ 添加新面容时上一次录入的图片 / 表单残留（页面被 keep-alive 缓存）→ 每次进入按当前操作重置。 |
...
| v0.5.9 | 2026-07-14 | 休眠/自动锁屏/录入/Passkey/Broker 综合修复 | **修复 #26 休眠唤醒后 LogonUI 卡死及手动 PIN 解锁后摄像头延迟释放**：用 `CancelSynchronousIo` 打断后台同步 I/O，release 与 CloseHandle 移到分离线程，宿主 UI 线程不再阻塞；无可用人脸记录时不再预热摄像头，手动 PIN 释放后等待凭据会话真正切换才恢复预热，杜绝旧会话同秒重开摄像头<br />**修复 #27 自动锁屏调用失败却被误报成功**：校验锁屏结果、重试并在持续失败时记录策略诊断与退避<br />**优化录入黑屏与开摄像头慢**：进入录入页即在后台加载模型并通过 `ui_release` 让后台预热释放设备，模型构建不再长期占用全局摄像头状态锁；实体摄像头连续取得 3 帧即可结束预热，虚拟摄像头仍保留最多 10 帧兼容回退；UI 与 Unlock 均在首次打开前关闭 MSMF 硬件帧转换，并按 MSMF → DShow → Any 回退<br />**修复 Passkey 卸载残留和 Geek 清理后备份丢失**：认证器主动反注册，备份迁移到 `%ProgramData%\facewinunlock-tauri\PasskeyBackup`<br />**无 UIA 的 Broker 分类**：订阅 WebAuthn 1000–1008 事务作为强否决信号，Passkey/security key/FIDO2/设置 PIN 跳过；Chrome/Edge 查看密码和健康监视下的网页填充走人脸，`CREDUI_BROWSER_PASSWORD_FILL=0` 可关闭 Unknown 兜底；彻底删除 UIA 与 PIN 自动注入运行代码 |
