...
| v0.5.3 | 2026-06-21 | 修复更新检查反复提示 + GPU 后端实验性警告 | **修复「已是最新版仍反复提示更新」**：更新检查读 `CARGO_PKG_VERSION`（`UI/src-tauri/Cargo.toml`），而发版只 bump 了 `tauri.conf.json`、漏了 Cargo.toml，导致 current_version 落后于 release tag；现已 bump 并让 CI 从 tag **同时**同步 `tauri.conf.json` 与 `Cargo.toml`，杜绝复发<br />**GPU（OpenCL / OpenCL FP16）推理后端实验性警告**（[issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3) 后续）：实测部分 Win10 设备选 OpenCL FP16 后人脸识别极慢（单次解锁约 90 秒）且匹配不上，表现为锁屏一直转圈不解锁、一致性检查黑屏卡住、占用偏高，而 CPU 后端正常；在「首选项」切到 GPU 后端时明确提示实验性并引导遇异常改回 CPU，一致性检查出错时也给出同样引导<br />**OpenCL kernel tuning 缓存优化**：Unlock 服务与 UI 启动时设置 `OPENCV_OCL4DNN_CONFIG_PATH` 到持久可写目录（SYSTEM 用 ProgramData、UI 用 LOCALAPPDATA）。此前不设该目录，OpenCV 的 ocl4dnn 每次 `forward` 都对卷积层重新做 OpenCL kernel 编译 + auto-tuning（GPU/FP16 后端首次推理可达 ~90s），且结果不持久化导致**每次解锁/识别都重复**；设置后只首次 tuning、后续从缓存秒加载。注：该优化只解决「慢」；若某显卡 FP16 精度不足导致「匹配不上」仍需改回 CPU<br />**Passkey 卸载默认保留通行密钥**（[issue #3](https://github.com/starnotes-xj/FaceWinUnlock-Tauri/issues/3)）：卸载/全量更新/重装后免重新注册——私钥在 Microsoft Software KSP（密钥名 `facewinunlock/<userId>`，per-user，本就不随 MSIX 卸载删除），凭据元数据改用 `Remove-AppxPackage -PreserveApplicationData` 保留 + 包外备份/恢复兜底；卸载提供「保留密钥」「彻底清除」二选项，核心/NSIS 卸载默认保留 |
