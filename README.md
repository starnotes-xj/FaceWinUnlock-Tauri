...
| v0.5.9 | 2026-07-13 | 休眠/自动锁屏/录入/Passkey/Broker 综合修复 | **修复 #26 休眠唤醒后 LogonUI 卡死及手动 PIN 解锁后摄像头延迟释放**：用 `CancelSynchronousIo` 打断后台同步 I/O，release 与 CloseHandle 移到分离线程，宿主 UI 线程不再阻塞<br />**修复 #27 自动锁屏调用失败却被误报成功**：校验锁屏结果、重试并在持续失败时记录策略诊断与退避<br />**修复录入黑屏与开摄像头慢**：录入前通过 `ui_release` 让后台预热释放设备；UI 与 Unlock 均在首次打开前关闭 MSMF 硬件帧转换，并按 MSMF → DShow → Any 回退<br />**修复 Passkey 卸载残留和 Geek 清理后备份丢失**：认证器主动反注册，备份迁移到 `%ProgramData%\facewinunlock-tauri\PasskeyBackup`<br />**无 UIA 的 Broker 分类**：订阅 WebAuthn 1000–1008 事务作为强否决信号，Passkey/security key/FIDO2/设置 PIN 跳过；Chrome/Edge 查看密码和健康监视下的网页填充走人脸，`CREDUI_BROWSER_PASSWORD_FILL=0` 可关闭 Unknown 兜底；彻底删除 UIA 与 PIN 自动注入运行代码 |
